...
func openEmail(t *testing.T, ctx *testContext, rid string) {
	resp, err := http.Get(fmt.Sprintf("%s/page?%s=%s", ctx.phishServer.URL, models.RecipientParameter, rid))
	if err != nil {
		t.Fatalf("error requesting /page endpoint: %v", err)
	}
	defer resp.Body.Close()
	got, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		t.Fatalf("error reading response body from /page endpoint: %v", err)
	}
...
func openEmail404(t *testing.T, ctx *testContext, rid string) {
	...
	if got != expected {
		t.Fatalf("invalid status code received for /page endpoint. expected %d got %d", expected, got)
	}
}
...
func TestNoRecipientID(t *testing.T) {
	ctx := setupTest(t)
	defer tearDown(t, ctx)
	resp, err := http.Get(fmt.Sprintf("%s/page", ctx.phishServer.URL))
	if err != nil {
		t.Fatalf("error requesting /page endpoint: %v", err)
	}
	got := resp.StatusCode
...
func TestRobotsHandler(t *testing.T) {
	...
	if got != expectedStatus {
		t.Fatalf("invalid status code received for /page endpoint. expected %d got %d", expectedStatus, got)
	}
...
func TestTransparencyRequest(t *testing.T) {
	...
	transparencyRequest(t, ctx, result, rid, "/page")
